...
18. **WEB INGEST EXTRACTION FIX PASS (T001-T006):**
    *   **T001 — Article body detection with density scoring:** Added `scoreContentDensity()` function (0-100 scale) that evaluates text blocks on: length (30pts), average sentence length (30pts), real paragraph count (20pts), factual signals/numbers (10pts), with penalties for boilerplate-heavy or nav-heavy content. All extraction passes now pick the candidate with highest density score rather than just the longest text.
    *   **T002 — Junk content filter:** Expanded `BOILERPLATE_PHRASES` to 30+ entries covering auth walls, paywall triggers, navigation/UI patterns, and ads. Added `HARD_REJECT_PHRASES` (any single occurrence marks content invalid). Added `NAV_SIGNAL_PHRASES` and `isNavOrSidebarContent()` function that rejects text with >4 nav signals in <1500 chars, >60% short-line ratio, or <2 real sentences for text >400 chars. Documents with invalid content get `[INVALID_CONTENT]` prefix in rawText — analysis is skipped for these.
    *   **T003 — Minimum content requirement:** `extractArticleText()` now requires: `text.length >= 500` AND at least one real sentence (40+ char clause ending in punctuation) — otherwise `contentQuality="invalid"`.
    *   **T004 — contentQuality field:** `ExtractionResult` now carries `contentQuality: "valid" | "partial" | "invalid"` and `densityScore: number`. Both are encoded in the `ATLAS-DIAG` prefix as `cq=` and `density=`. `parseAtlasDiag()` decodes them. Only docs with `cq !== "invalid"` proceed to entity extraction.
    *   **T005 — Div density fallback (Pass 2.5):** After selector and paragraph-agg passes, if density score is still <35, iterates ALL div/section blocks, scores each by paragraph density, picks best candidate that is >400 chars and has real sentence structure. Prevents nav-only content from passing through.
    *   **T006 — Validated against case 39:** Case 39 restocked with 3 real-content docs from lahsa.org (cq=valid), lacounty.gov (cq=partial), and dailynews.com (cq=partial). Result: LAHSA promoted as primary entity, 6 entities total, case health moved from SPARSE → DEVELOPING, quality WEAK → MODERATE, confidence 21 → 57.
